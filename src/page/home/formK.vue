<template>
  <div class="w-100 order flex-col">
          ...
          >
            ...
            <el-table-column prop="employeeName" label="操作">
              <template slot-scope="scope">
                <span
                  style="color:#0d906e;cursor:pointer;"
                  v-if="scope.row.percentageStatus == 1"
                  @click.stop="settingRow(scope.row, 0)"
                >已设置</span>
                <span
                  style="color:red;cursor:pointer;"
                  v-else
                  @click.stop="settingRow(scope.row, 1)"
                >未设置</span>
              </template>
            </el-table-column>
          </el-table>
            ...
            <el-table-column prop="employeeName" label="操作">
              <template slot-scope="scope">
                <el-button
                  type="success"
                  size="mini"
                  :disabled="selectOrder.percentageStatus == 1"
                  @click="royaltyConfim(scope.row.sourceType, scope.row)"
                >设置</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog
      :title="royaltyInfo.royaltyName + '提成设置'"
    ...
    >
      ...
      <div
        ...
        v-if="royaltyInfo.percentage && royaltyInfo.percentage.percentageWay"
      >
        <el-row :gutter="20">
          ...
          <el-col :span="19">
            <span>
              提成方式：
              <font color="#0d906e">{{royaltyInfo.percentage.percentageWay == 1 ? '提成点' : '固定'}}</font>
            </span>
            <span style="padding-left: 20px;">
              是否按职务提成：
              <font color="#0d906e">{{royaltyInfo.percentage.isByPosition == true ? '是' : '否'}}</font>
            </span>
            <span
              style="padding-left: 20px;"
              v-if="royaltyInfo.percentage.isByPosition != true && royaltyInfo.percentage.percentageMethod"
            >
              分成方式：
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageMethod == 1 ? '总额均分' : '等额提成'}}</font>
            </span>
            <br />
            <span
              style="padding-top: 20px;"
              v-if="royaltyInfo.percentage.percentagePoint || royaltyInfo.percentage.percentageAmout"
            >
              所有人：
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageWay == 1 ? royaltyInfo.percentage.percentagePoint + '%' : royaltyInfo.percentage.percentageAmout + '元'}}</font>
            </span>
            <span
              style="padding-top: 20px;"
              v-if="royaltyInfo.percentage.percentagePointYW || royaltyInfo.percentage.percentageAmoutYW"
            >
              业务员：
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageWay == 1 ? royaltyInfo.percentage.percentagePointYW + '%' : royaltyInfo.percentage.percentageAmoutYW + '元'}}</font>
            </span>
            <span
              style="padding-left: 20px;"
              v-if="royaltyInfo.percentage.percentagePointJD || royaltyInfo.percentage.percentageAmoutJD"
            >
              接待员：
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageWay == 1 ? royaltyInfo.percentage.percentagePointJD + '%' : royaltyInfo.percentage.percentageAmoutJD + '元'}}</font>
            </span>
            <span
              style="padding-left: 20px;"
              v-if="royaltyInfo.percentage.percentagePointCW || royaltyInfo.percentage.percentageAmoutCW"
            >
              施工员：
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageWay == 1 ? royaltyInfo.percentage.percentagePointCW + '%' : royaltyInfo.percentage.percentageAmoutCW + '元'}}</font>
            </span>
            <span
              style="padding-left: 20px;"
              v-if="royaltyInfo.percentage.percentagePointSG || royaltyInfo.percentage.percentageAmoutSG"
            >
              主管:
              <font
                color="#0d906e"
              >{{royaltyInfo.percentage.percentageWay == 1 ? royaltyInfo.percentage.percentagePointSG + '%' : royaltyInfo.percentage.percentageAmoutSG + '元'}}</font>
            </span>
          </el-col>
        </el-row>
      </div>
      <div class="royalty" v-for="(item, index) in royaltyForm.percentages" :key="index">
        <el-row :gutter="20">
          <el-col :span="12">
            {{item.employeeName}}
			<span v-if="item.role == 0">(老板)</span>
			<span v-if="item.role == 1">(接待员)</span>
			<span v-if="item.role == 2">(财务)</span>
			<span v-if="item.role == 3">(施工员)</span>
			<span v-if="item.role == 4">(业务员)</span>
            <!-- <span
              v-if="royaltyInfo.percentage && royaltyInfo.percentage.isByPosition == true"
            >（{{item.roleName}}）</span> -->
          </el-col>
    ...
    <el-dialog title="选择员工" :visible.sync="userVisible" center :modal="false" >
      <el-table :data="userData" @selection-change="selectUser"  :max-height="380">
...
export default {
  ...
  methods: {
    ...
    addPercentage() {
		...
		console.log(this.royaltyForm.percentages )
      ...
      this.selectedUser.forEach(v => {
        ...
        if (percentage && percentage.percentageWay) {
          ...
          if (percentage.isByPosition == true) {
            ...
            if (percentage.percentageWay == 2) {
              //固定
              if (percentage.percentageMethod == 1) {
                //总额均分
                const price = (
                  percentage.percentageAmout / this.selectedUser.length
                ).toFixed(2);
              ...
              if (percentage.percentageMethod == 1) {
                ...
                v.percentagePoint = (
                  percentage.percentagePoint / this.selectedUser.length
                ).toFixed(2);
              } else {
          ...
          if (percentage.isByPosition == true) {
            ...
            if (percentage.percentageWay == 2) {
              ...
              if (percentage.percentageMethod == 1) {
                //总额均分
                const price = (
                  percentage.percentageAmout /
                  this.royaltyForm.percentages.length
                ).toFixed(2);
                v.percentageAmout = price;
              } else {
                v.percentageAmout = percentage.percentageAmout.toFixed(2);
